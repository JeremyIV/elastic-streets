...
v_corr = chord[:, None] / np.maximum(ew - offsets[None, :], 30.0) * 3.6  # kph
is_fwy_corr = (ii < NFWY) & (jj < NFWY)
print(f"{E} corridors ({is_fwy_corr.sum()} freeway-class)")

# ---- street graph with class-aware corridor speeds ----
...
for u, v, k, d in G.edges(keys=True, data=True):
    ...
    mid = pts.mean(0)
    if fwy:
        _, ce = tree_f.query(mid)
        ce = map_f[ce]
    else:
        _, ce = tree_s.query(mid)
        ce = map_s[ce]
    ea.append(idx[u]); eb.append(idx[v])
    elen.append(float(d["length"]))
    geom.append(pts)
    espd.append(v_corr[ce])
...
num = den = 0.0
L0all = np.sqrt(((Pg[A0] - Pg[B0]) ** 2).sum(1))
for T in T_hours:
    Tij = T.ravel()
    kp = np.isfinite(Tij) & (Tij > 30) & (A0 != B0)
    num += (L0all[kp] * Tij[kp]).sum()
    den += (Tij[kp] ** 2).sum()
c = num / den
